...
This module contains the functionality to test the churn_library.py. With each functions of churn_library.py is tetested here by try assert block which logs error and info for each case 
...
def test_import():
    ...
    '''
    #Test for CSV import successfull
    try:
        ...
        raise err
    #Test for dataframe imported successfull
    try:
        assert df.shape[0] > 0
        assert df.shape[1] > 0
...
def test_encoder_helper():
    ...
    try:
        ...
        # If both data frame are same in lenght column      
        assert len(
